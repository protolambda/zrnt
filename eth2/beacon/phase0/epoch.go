...
type EpochProcess struct {
	...
	IndicesToSlash                    []common.ValidatorIndex
...
func PrepareEpochProcess(ctx context.Context, spec *common.Spec, epc *EpochsContext, state *BeaconStateView) (out *EpochProcess, err error) {
	...
	for i := common.ValidatorIndex(0); true; i++ {
		...
		if status.Active && flat.EffectiveBalance <= spec.EJECTION_BALANCE && flat.ExitEpoch == common.FAR_FUTURE_EPOCH {
	...
	for i := common.ValidatorIndex(0); i < common.ValidatorIndex(count); i++ {
		status := &out.Statuses[i]
