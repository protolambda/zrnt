...
var SuffixStateDiffType = ContainerType("SuffixStateDiff", []FieldDef{
	{"suffix", ByteType},
	{"current_value", UnionType([]TypeDef{nil, common.Bytes32Type})},
	{"new_value", UnionType([]TypeDef{nil, common.Bytes32Type})},
})
...
var IpaProofType = ContainerType("IpaProof", []FieldDef{
	{"C_L", VectorType(common.Bytes32Type, IPA_PROOF_DEPTH)},
	{"C_R", VectorType(common.Bytes32Type, IPA_PROOF_DEPTH)},
	{"final_evaluation", common.Bytes32Type},
})
...
var VerkleProofFields = []FieldDef{
	...
	{"commitments_by_path", ListType(common.Bytes32Type, MAX_STEMS*MAX_COMMITMENTS_PER_STEM)},
	{"D", common.Bytes32Type},
...
func (v *ExecutionPayloadHeaderView) Raw() (*ExecutionPayloadHeader, error) {
	...
	executionWitnessRoot, err := AsRoot(values[__verkleproofRoot], err)
	if err != nil {
		return nil, err
	}
...
type ExecutionPayloadHeader struct {
	...
	ExecutionWitnessRoot common.Root        `json:"execution_witness_root" yaml:"execution_witness_root"`
}
...
func (s *ExecutionPayloadHeader) View() *ExecutionPayloadHeaderView {
	...
	v, err := AsExecutionPayloadHeader(ExecutionPayloadHeaderType.FromFields(pr, cb, sr, rr, lb, rng, nr, gl, gu, ts, ed, bf, bh, tr, wr, ew))
	if err != nil {
		panic(err)
	}
	return v
...
func (s *ExecutionPayloadHeader) Deserialize(dr *codec.DecodingReader) error {
	return dr.Container(&s.ParentHash, &s.FeeRecipient, &s.StateRoot,
		...
		&s.WithdrawalsRoot, &s.ExecutionWitnessRoot,
	)
}
...
func (s *ExecutionPayload) Deserialize(spec *common.Spec, dr *codec.DecodingReader) error {
	return dr.Container(&s.ParentHash, &s.FeeRecipient, &s.StateRoot,
		...
		spec.Wrap(&s.Withdrawals), &s.ExecutionWitness,
	)
}
