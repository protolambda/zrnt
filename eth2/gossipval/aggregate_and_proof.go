...
type AggregateSeenFn func(aggRoot beacon.Root) bool

// Checks if an aggregate by the given aggregator for the given epoch has been seen before.
type AggregatorSeenFn func(targetEpoch beacon.Epoch, aggregator beacon.ValidatorIndex) bool

func (gv *GossipValidator) ValidateAggregateAndProof(ctx context.Context, signedAgg *beacon.SignedAggregateAndProof,
	aggAttSeenFn AggregateSeenFn, aggregatorSeenFn AggregatorSeenFn, isBadBlock IsBadBlockFn) GossipValidatorResult {
	...
	if epoch, index := att.Data.Target.Epoch, signedAgg.Message.AggregatorIndex; aggregatorSeenFn(epoch, index) {
	...
	// (via aggregate gossip, within a verified block, or through the creation of an equivalent aggregate locally).
	aggRoot := att.HashTreeRoot(gv.Spec, tree.GetHashFn())
	if aggAttSeenFn(aggRoot) {
	...
	startSlot, _ := gv.Spec.EpochStartSlot(att.Data.Target.Epoch)

	towardsCtx, cancel := context.WithTimeout(ctx, catchupTimeout)
	defer cancel()

	entry, err := gv.Chain.Towards(towardsCtx, att.Data.Target.Root, startSlot)
	if err != nil {
	...
	if valid, err := gv.Spec.ValidateAggregateSelectionProof(epc, state, att.Data.Slot, att.Data.Index, signedAgg.Message.AggregatorIndex, signedAgg.Message.SelectionProof); err != nil {
		return GossipValidatorResult{IGNORE, err}
	} else if !valid {
	...
	dom, err := state.GetDomain(gv.Spec.DOMAIN_AGGREGATE_AND_PROOF, att.Data.Target.Epoch)
	if err != nil {
		return GossipValidatorResult{IGNORE, err}
	}
	sigRoot := beacon.ComputeSigningRoot(signedAgg.Message.HashTreeRoot(gv.Spec, tree.GetHashFn()), dom)
	...
	if indexedAtt, err := att.ConvertToIndexed(gv.Spec, committee); err != nil {
		// it should always convert.
		// Something is very wrong if not, e.g. bad bitfield length.
		return GossipValidatorResult{REJECT, err}
	} else if err := gv.Spec.ValidateIndexedAttestation(epc, state, indexedAtt); err != nil {
		return GossipValidatorResult{REJECT, err}
	}
