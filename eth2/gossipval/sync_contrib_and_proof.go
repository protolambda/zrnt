...
func ValidateSyncContribAndProof(ctx context.Context, signedContribAndProof *altair.SignedContributionAndProof,
	...
	if err := CheckSlotSpan(scpVal.SlotAfter, contrib.Slot, 1); err != nil {
		return GossipValidatorResult{IGNORE, fmt.Errorf("contribution not for current slot: %v", err)}
	}
	...
	if count := contrib.AggregationBits.OnesCount(); count == 0 {
		return GossipValidatorResult{REJECT, fmt.Errorf("sync committee contribution needs to have at least 1 participant")}
	}
	...
	if !altair.IsSyncCommitteeAggregator(spec, contribAndProof.SelectionProof) {
		return GossipValidatorResult{REJECT, errors.New("invalid sync aggregator selection proof")}
	}

	ch := scpVal.Chain()
	entry, ok := ch.ByBlockSlot(contrib.BeaconBlockRoot, contrib.Slot)
	if !ok {
		return GossipValidatorResult{IGNORE, fmt.Errorf("cannot find beacon block that sync contribution contributes to")}
	}
	...
	if err != nil {
		...
		if !found {
			return GossipValidatorResult{REJECT, fmt.Errorf("could not find aggregator %d in sync sub-committee %d at block %s",
	...
	if scpVal.SeenContribution(contribAndProof.AggregatorIndex, contrib.Slot, uint64(contrib.SubcommitteeIndex)) {
		return GossipValidatorResult{IGNORE, fmt.Errorf("already seen conribution of aggregator %d at slot %d for sync subnet %d",
	...
	if err := altair.ValidateSyncAggregatorSelectionProof(spec, epc, scpVal.GetDomain,
		...
		return GossipValidatorResult{REJECT, fmt.Errorf("invalid sync agg selection proof: %v", err)}
	}

	// [REJECT] The aggregator signature, signed_contribution_and_proof.signature, is valid.
	if err := signedContribAndProof.VerifySignature(spec, epc, scpVal.GetDomain); err != nil {
		return GossipValidatorResult{REJECT, fmt.Errorf("invalid sync contribution aggregator signature: %v", err)}
	}
	...
	if err := contribAndProof.Contribution.VerifySignature(spec, pubs, scpVal.GetDomain); err != nil {
		return GossipValidatorResult{REJECT, fmt.Errorf("invalid sync contribution signature: %v", err)}
	}
